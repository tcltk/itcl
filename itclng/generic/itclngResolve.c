 ...
 * ItclngVarLookup info needed at runtime.
 */
typedef struct ItclngResolvedVarInfo {
    Tcl_ResolvedVarInfo vinfo;        /* This must be the first element. */
    ItclngVarLookup *vlookup;           /* Pointer to lookup info. */
} ItclngResolvedVarInfo;

static Tcl_Var ItclngClassRuntimeVarResolver _ANSI_ARGS_((
    Tcl_Interp *interp, Tcl_ResolvedVarInfo *vinfoPtr));


/*
 * ------------------------------------------------------------------------
 *  Itclng_ClassCmdResolver()
 *
    ...
    Tcl_Command *rPtr)		/* returns: resolved command */
{
     ...
     *
     *  TRICKY NOTE:  Use Itclng_GetTrueNamespace to determine
    ...
    if (imPtr->protection != ITCLNG_PUBLIC) {
        ...
        if (!Itclng_CanAccessFunc(imPtr, nsPtr)) {
    ...
    Tcl_Var *rPtr)            /* returns: resolved variable */
{
    ...
    assert(Itclng_IsClassNamespace(nsPtr));
    ...
    if ((strstr(name,"::") == NULL) &&
            Itclng_IsCallFrameArgument(interp, name)) {
        return TCL_CONTINUE;
    }
     ...
     */
    if ((vlookup->ivPtr->flags & ITCLNG_COMMON) != 0) {
	...
	Tcl_DStringAppend(&buffer, "::this", 6);
        Tcl_Var varPtr;
	varPtr = Itclng_FindNamespaceVar(interp, Tcl_DStringValue(&buffer), NULL, 0);
        if (varPtr != NULL) {
            *rPtr = varPtr;
	    return TCL_OK;
        }
...
/*
 * ------------------------------------------------------------------------
 *  Itclng_ClassCompiledVarResolver()
 *
 ...
 *  by their ItclngVarLookup record.  At runtime, Tcl constructs the
 *  compiled local variables by calling ItclngClassRuntimeVarResolver.
 *
 *  If a variable is found, this procedure returns TCL_OK along with
                                 ...
                                 *   resolve the variable at runtime */
{
    ...
    /*
     *  Return the ItclngVarLookup record.  At runtime, Tcl will
     *  call ItclngClassRuntimeVarResolver with this record, to
     *  plug in the appropriate variable for the current object
     *  context.
     */
    (*rPtr) = (Tcl_ResolvedVarInfo *) ckalloc(sizeof(ItclngResolvedVarInfo));
    (*rPtr)->fetchProc = ItclngClassRuntimeVarResolver;
    (*rPtr)->deleteProc = NULL;
    ((ItclngResolvedVarInfo*)(*rPtr))->vlookup = vlookup;
 ...
 *  Itclng_ClassCompiledVarResolver.  Returns the Tcl_Var representation
 *  for the data member.
 * ------------------------------------------------------------------------
 */
static Tcl_Var
ItclngClassRuntimeVarResolver(
    Tcl_Interp *interp,               /* current interpreter */
    Tcl_ResolvedVarInfo *resVarInfo)  /* contains ItclngVarLookup rep
                                       * for variable */
{
    ItclngVarLookup *vlookup = ((ItclngResolvedVarInfo*)resVarInfo)->vlookup;
    ...
    ItclngCallContext *callContextPtr;
    callContextPtr = Itclng_PeekStack(&iclsPtr->infoPtr->contextStack);
    if (callContextPtr == NULL) {
        return NULL;
    }
    if (callContextPtr->ioPtr == NULL) {
    ...
    if (contextIoPtr != NULL) {
	    ...
	    Tcl_DStringAppend(&buffer, "::this", 6);
            Tcl_Var varPtr;
	    varPtr = Itclng_FindNamespaceVar(interp, Tcl_DStringValue(&buffer),
	            NULL, 0);
            if (varPtr != NULL) {
	        return varPtr;
            }
...
{
    ItclngObjectInfo *infoPtr = (ItclngObjectInfo*)contextNs->clientData;
    ItclngClass *iclsPtr = (ItclngClass*)Itclng_PeekStack(&infoPtr->clsStack);
    ...
    if (hPtr) {
        ...
        if ((vlookup->ivPtr->flags & ITCLNG_COMMON) != 0) {
            if (!vlookup->accessible) {
                Tcl_AppendResult(interp,
                    "can't access \"", name, "\": ",
                    Itclng_ProtectionStr(vlookup->ivPtr->protection),
                    " variable",
                    (char*)NULL);
                return TCL_ERROR;
            }
...
int
ItclngSetParserResolver(
    Tcl_Namespace *nsPtr)
{
    Itclng_SetNamespaceResolvers(nsPtr, (Tcl_ResolveCmdProc*)NULL,
            Itclng_ParseVarResolver, (Tcl_ResolveCompiledVarProc*)NULL);
    return TCL_OK;
}
