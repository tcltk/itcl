 ...
 *  either be declared via Itclng_RegisterC() or dynamically loaded.
 *
 * ========================================================================
 *  AUTHOR:  Michael J. McLennan
 *           Bell Labs Innovations for Lucent Technologies
 *           mmclennan@lucent.com
 *           http://www.tcltk.com/itclng
 *
 *  overhauled version author: Arnulf Wiedemann
 *
 *     RCS:  $Id: itclngLinkage.c,v 1.1.2.1 2008/02/10 18:35:20 wiede Exp $
 * ========================================================================
...
static Tcl_HashTable* ItclngGetRegisteredProcs _ANSI_ARGS_((Tcl_Interp *interp));
static void ItclngFreeC _ANSI_ARGS_((ClientData clientData, Tcl_Interp *interp));
 ...
 */
int
Itclng_RegisterC(interp, name, proc, clientData, deleteProc)
    ...
    Tcl_CmdDeleteProc *deleteProc;  /* proc called to free up client data */
{
     ...
     *  with the same proc.
     */
    procTable = ItclngGetRegisteredProcs(interp);
    entry = Tcl_CreateHashEntry(procTable, name, &newEntry);
    if (!newEntry) {
        cfunc = (ItclngCfunc*)Tcl_GetHashValue(entry);
        ...
        cfunc = (ItclngCfunc*)ckalloc(sizeof(ItclngCfunc));
 ...
 *  list of procedures registered previously by either Itclng_RegisterC
 *  or Itclng_RegisterObjC and returns pointers to the appropriate
    ...
    *cDataPtr   = NULL;
    ...
    if (interp) {
        procTable = (Tcl_HashTable*)Tcl_GetAssocData(interp,
            "itclng_RegC", (Tcl_InterpDeleteProc**)NULL);
...
ItclngGetRegisteredProcs(interp)
    Tcl_Interp *interp;  /* interpreter handling this registration */
{
    Tcl_HashTable* procTable;
     ...
     */
    procTable = (Tcl_HashTable*)Tcl_GetAssocData(interp, "itclng_RegC",
        (Tcl_InterpDeleteProc**)NULL);

    if (!procTable) {
        procTable = (Tcl_HashTable*)ckalloc(sizeof(Tcl_HashTable));
        Tcl_InitHashTable(procTable, TCL_STRING_KEYS);
        Tcl_SetAssocData(interp, "itclng_RegC", ItclngFreeC,
 ...
 *  free up the associated data created by Itclng_RegisterC and
 *  Itclng_RegisterObjC.
 * ------------------------------------------------------------------------
 */
static void
ItclngFreeC(clientData, interp)
