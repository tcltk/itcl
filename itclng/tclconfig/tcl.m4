...
# RCS: @(#) $Id: tcl.m4,v 1.1.2.1 2008/01/12 18:45:32 wiede Exp $
#
...
#
#	Defines the following vars:
#		TCLOO_LIB_DIR	Full path to the directory containing
...
AC_DEFUN([TEA_PATH_TCLOOCONFIG], [
	...
	AC_CACHE_VAL(ac_cv_c_tclooconfig,[
	    ...
	    # check in a few other private locations
	    if test x"${ac_cv_c_tclcooonfig}" = x ; then
		for i in \
			${srcdir}/../oo \
	...
	if test x"${ac_cv_c_tclooconfig}" = x ; then
	    TCLOO_LIB_DIR="# no TclOO configs found"
	    AC_MSG_WARN([Can't find TclOO configuration definitions])
	    exit 0
	else
	    no_tcl=
	    TCLOO_LIB_DIR=${ac_cv_c_tclooconfig}
	    AC_MSG_RESULT([found ${TCLOO_LIB_DIR}/tclooConfig.sh])
	fi
    fi
])
...
AC_DEFUN([TEA_LOAD_TCLOOCONFIG], [
    AC_MSG_CHECKING([for existence of ${TCLOO_LIB_DIR}/tclooConfig.sh])

    if test -f "${TCLOO_LIB_DIR}/tclooConfig.sh" ; then
        AC_MSG_RESULT([loading])
	. "${TCLOO_LIB_DIR}/tclooConfig.sh"
    else
        AC_MSG_RESULT([could not find ${TCLOO_LIB_DIR}/tclooConfig.sh])
    fi

    # eval is required to do the TCL_DBGX substitution
    eval "TCLOO_LIB_SPEC=\"${TCLOO_LIB_SPEC}${SHARED_LIB_SUFFIX}\""
    ...
    eval "TCLOO_CFLAGS=\"${TCLOO_CFLAGS}\""

    # If the TCLOO_LIB_DIR is the build directory (not the install directory),
    ...
    if test -f "${TCLOO_LIB_DIR}/Makefile" ; then
...
#        TCLOO_STUB_LIB_SPEC=${TCLOO_BUILD_STUB_LIB_SPEC}
	...
	case ${TCLOO_DEFS} in
	    *TCL_FRAMEWORK*)
		if test -f "${TCLOO_LIB_DIR}/${TCLOO_LIB_FILE}"; then
		    for i in "`cd ${TCLOO_LIB_DIR}; pwd`" \
			     "`cd ${TCLOO_LIB_DIR}/../..; pwd`"; do
		...
		if test -f "${TCLOO_BIN_DIR}/${TCLOO_STUB_LIB_FILE}"; then
		    TCLOO_STUB_LIB_PATH="${TCLOO_LIB_DIR}/${TCLOO_STUB_LIB_FILE}"
		fi
		;;
	esac
    fi
    ...
    # eval is required to do the TCL_DBGX substitution
    eval "TCLOO_LIB_DIR=\"${TCLOO_LIB_DIR}\""
...
AC_DEFUN([TEA_MAKE_LIB], [
    ...
    if test "${TEA_PLATFORM}" = "windows" ; then
	if test "${SHARED_BUILD}" = "1" ; then
	    if test x"${ITCLNG_LIB_DIR}" != x ; then
		SHLIB_LD_LIBS="${SHLIB_LD_LIBS} ${ITCLNG_LIB_SPEC}"
	    fi
	    # We force the unresolved linking of symbols that are really in
	    # the private libraries of Tcl and Tk.
	    SHLIB_LD_LIBS="${SHLIB_LD_LIBS} \"`${CYGPATH} ${TCL_BIN_DIR}/${TCL_STUB_LIB_FILE}`\""
	    if test x"${ITCLNG_LIB_DIR}" != x ; then
		SHLIB_LD_LIBS="${SHLIB_LD_LIBS} \"`${CYGPATH} ${ITCLNG_LIB_DIR}/${ITCLNG_STUB_LIB_SPEC}`\""
	    fi
	...
	if test "${SHARED_BUILD}" = "1" ; then
	    ...
	    if test x"${ITCLNG_LIB_DIR}" != x ; then
		SHLIB_LD_LIBS="${SHLIB_LD_LIBS} ${ITCLNG_STUB_LIB_SPEC}"
	    fi
...
#------------------------------------------------------------------------
# TEA_PRIVATE_TCLOO_HEADERS --
#
#	Locate the private TclOO include files
#
# Arguments:
#
#	Requires:
#		TCLOO_SRC_DIR	Assumes that TEA_LOAD_TCLOOCONFIG has
#				 already been called.
#
# Results:
#
...
AC_DEFUN([TEA_PRIVATE_TCLOO_HEADERS], [
    AC_MSG_CHECKING([for TclOO private include files])

    TCLOO_SRC_DIR_NATIVE=`${CYGPATH} ${TCLOO_SRC_DIR}`
    TCLOO_TOP_DIR_NATIVE=\"${TCLOO_SRC_DIR_NATIVE}\"
    TCLOO_UNIX_DIR_NATIVE=\"${TCLOO_SRC_DIR_NATIVE}/unix\"
    TCLOO_WIN_DIR_NATIVE=\"${TCLOO_SRC_DIR_NATIVE}/win\"
    TCLOO_GENERIC_DIR_NATIVE=\"${TCLOO_PRIVATE_INCLUDE_SPEC}\"
    TCLOO_XLIB_DIR_NATIVE=\"${TCLOO_SRC_DIR_NATIVE}/xlib\"
    if test "${TEA_PLATFORM}" = "windows"; then
	TCLOO_PLATFORM_DIR_NATIVE=${TCLOO_WIN_DIR_NATIVE}
    else
	TCLOO_PLATFORM_DIR_NATIVE=${TCLOO_UNIX_DIR_NATIVE}
    fi
    # We want to ensure these are substituted so as not to require
    # any *_NATIVE vars be defined in the Makefile
    TCLOO_INCLUDES="${TCLOO_PRIVATE_INCLUDE_SPEC} -I${TCLOO_PLATFORM_DIR_NATIVE}"
    if test "${TEA_WINDOWINGSYSTEM}" = "win32" \
	-o "${TEA_WINDOWINGSYSTEM}" = "aqua"; then
	TCLOO_INCLUDES="${TCLOO_INCLUDES}"
    fi
    if test "${TEA_WINDOWINGSYSTEM}" = "aqua"; then
	TCLOO_INCLUDES="${TCLOO_INCLUDES} -I${TCLOO_SRC_DIR_NATIVE}/macosx"
    fi
    if test "`uname -s`" = "Darwin"; then
        # If TCLOO was built as a framework, attempt to use
        # the framework's Headers and PrivateHeaders directories
        case ${TCLOO_DEFS} in
	    *TCLOO_FRAMEWORK*)
	        if test -d "${TCLOO_LIB_DIR}/Headers" -a -d "${TCLOO_LIB_DIR}/PrivateHeaders"; then
	        TCLOO_INCLUDES="-I\"${TCLOO_LIB_DIR}/Headers\" -I\"${TCLOO_LIB_DIR}/PrivateHeaders\" ${TCLOO_INCLUDES}"; fi
	        ;;
	esac
    fi

    AC_SUBST(TCLOO_TOP_DIR_NATIVE)
    AC_SUBST(TCLOO_UNIX_DIR_NATIVE)
    AC_SUBST(TCLOO_WIN_DIR_NATIVE)
    AC_SUBST(TCLOO_GENERIC_DIR_NATIVE)
    AC_SUBST(TCLOO_PLATFORM_DIR_NATIVE)

    AC_SUBST(TCLOO_INCLUDES)
    AC_MSG_RESULT([Using srcdir found in tclooConfig.sh: ${TCLOO_SRC_DIR}])
])
...
# Results:
#
#	Adds a --with-tclooinclude switch to configure.
#	Result is cached.
#
#	Substs the following vars:
#		TCLOO_INCLUDES
...
AC_DEFUN([TEA_PUBLIC_TCLOO_HEADERS], [
	...
	if test x"${with_tclooinclude}" != x ; then
		...
		AC_MSG_ERROR([${with_tclooinclude} directory does not contain tclOO.h])
	    fi
	else
	    if test "`uname -s`" = "Darwin"; then
		# If Tk was built as a framework, attempt to use
		...
		case ${TCLOO_DEFS} in
		    *TCLOO_FRAMEWORK*)
			list="`ls -d ${TCLOO_LIB_DIR}/Headers 2>/dev/null`"
			;;
		esac
	    fi
	    ...
	    # relative to directory of tclooConfig.sh, Tcl's --prefix location, 
	    # relative to directory of tclConfig.sh.
		...
		`ls -d ${TCLOO_LIB_DIR}/../include  2>/dev/null` \
		`ls -d ${TCLOO_PREFIX}/include     2>/dev/null` \
    ...
    TCLOO_INCLUDE_DIR_NATIVE=`${CYGPATH} ${ac_cv_c_tclooh}`

    TCLOO_INCLUDES=-I\"${TCLOO_INCLUDE_DIR_NATIVE}\"
