    ...
    ItclObjectInfo *infoPtr)
{
    ...
    Tcl_CreateObjCommand2(interp, "::itcl::builtin::chain", Itcl_BiChainCmd2,
	    NULL, NULL);
...
{
    return Itcl_BiIsaCmd2(clientData, interp, objc, objv);
}
...
Itcl_BiConfigureCmd(
    ...
    Tcl_Obj *const objv[])   /* argument objects */
{
    return Itcl_BiConfigureCmd2(clientData, interp, objc, objv);
}
...
configureDone:
	...
	Tcl_Free ((char *)infoPtr->unparsedObjv);
...
Itcl_BiCgetCmd(
    ...
    Tcl_Obj *const objv[])   /* argument objects */
{
    return Itcl_BiCgetCmd2(clientData, interp, objc, objv);
}
    ...
    Tcl_Obj *const objv[])   /* argument objects */
{
	...
	for (idx = 2; idx < objc; idx++) {
	    ...
	    hPtr2 = Tcl_CreateHashEntry(&ioPtr->objectDelegatedOptions,
		    (char *)objv[idx], &isNew);
	    if (isNew) {
		idoPtr = (ItclDelegatedOption *)Tcl_Alloc(sizeof(
			ItclDelegatedOption));
