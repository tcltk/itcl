 ...
 *
 *     RCS:  $Id: itclBuiltin.c,v 1.1.2.36 2008/11/07 23:10:04 wiede Exp $
 * ========================================================================
...
//    char *token;
     ...
     *  context.
     */
    ItclShowArgs(0, "Itcl_BiMyTypeMethodCmd", objc, objv);
    contextIclsPtr = NULL;
    if (Itcl_GetContext(interp, &contextIclsPtr, &contextIoPtr) != TCL_OK) {
        return TCL_ERROR;
    }
    ...
    Tcl_Obj *CONST objv[])   /* argument objects */
{
	...
	Tcl_ListObjAppendElement(interp, resultPtr,
	        Tcl_NewStringObj("::itcl::callinstance", -1));
    ...
    Tcl_Obj *CONST objv[])   /* argument objects */
{
    ...
    resultPtr = Tcl_NewStringObj(contextIclsPtr->nsPtr->fullName, -1);
    Tcl_IncrRefCount(resultPtr);
    Tcl_AppendToObj(resultPtr, "::", -1);
    Tcl_AppendToObj(resultPtr, Tcl_GetString(objv[1]), -1);
    for (i = 2; i < objc; i++) {
        Tcl_AppendToObj(resultPtr, " ", -1);
        Tcl_AppendToObj(resultPtr, Tcl_GetString(objv[i]), -1);
    }
    Tcl_SetObjResult(interp, resultPtr);
    return TCL_OK;
}
